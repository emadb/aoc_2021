defmodule Aoc.DayFour do
  ...
  def part_two(file_path) do
    ...
    sum = card |> Enum.reject(fn {_, h} -> h == 1 end) |> Enum.reduce(0, fn {x, 0}, acc -> acc + x end)
    n * sum
  end
  ...
  defp play([n | rest], cards) do
    ...
    next_cards = Enum.map(cards, fn {w, nn, card} ->
      if w > 0 do
        {w, nn, card}
      else
  ...
  defp check_winner(cards) do
    Enum.map(cards, fn c ->
      case [rows(c), columns(c)] do
        [false, false] -> 0
        [_, _] -> Enum.filter(c, fn {_, t} -> t == 0 end) |> Enum.reduce(0, fn {x, 0}, acc -> x + acc end )
      end
    end)
  end
...
defmodule FF do
  ...
  def transpose(card) do
    Enum.flat_map([FF.get_column_0(card),
    FF.get_column_1(card),
    FF.get_column_2(card),
    FF.get_column_3(card),
    FF.get_column_4(card)], fn x-> x end)
  end
end
